...
ThermalScapeR pulls several of its raw inputs directly from Google Earth
Engine (GEE):
...
ThermalScapeR calls Earth Engine through `reticulate`, which drives a
...
Every session that calls a GEE-backed ThermalScapeR function needs this
...
**Why this works without a package-level init function**: ThermalScapeR
...
yourself, as shown above, is what makes every ThermalScapeR GEE function
work — there is no separate ThermalScapeR-specific setup call.

## 4. Google Drive authentication (for polling GEE exports)

GEE image exports land in a Google Drive folder before ThermalScapeR
