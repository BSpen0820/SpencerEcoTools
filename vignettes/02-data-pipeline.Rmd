...
summarize_climate_normals(
  dates      = as.Date(c("2019-07-01", "2020-07-01", "2021-07-01")),
  aorc_dir   = "./Data/Weather",
...
Passing `dtm_fine` here is optional but cheap insurance: it surfaces any
grid mismatch (resolution/CRS/origin) between the packaged veg/soil output
and the fine DEM as an early warning here, rather than only as a hard
`stop()` later inside `run_micro_big_nichemap()`'s pre-flight check.

This produces `./Data/VegPara/UI_Arboretum_VegPara_20230701_to_20230801.RDS`
and the equivalent `SoilPara` file, following the
`{study_area}_VegPara_{period_label}.RDS` naming convention (period labels
are always `YYYYMMDD_to_YYYYMMDD`). `package_veg_soil()` and
`package_climate()` build this label directly from `Start_Dates`/
`End_Dates` as given — that's why `End_Dates` is set to the first of the
month (`2023-08-01`) here, rather than the month's last day: downstream,
`run_micro_big_nichemap()` and `stitch_tiles_runmicro()` snap both dates
to the first of their month before looking up these same files, so the
raw dates used here need to already be on that footing for the filenames
to match.
...
```{r}
veg_template <- terra::unwrap(readRDS(
  "./Data/VegPara/UI_Arboretum_VegPara_20230701_to_20230801.RDS"
)$hgt)
...
This produces `./Data/Weather_Pkg/UI_Arboretum_Climate_20230701_to_20230801.RDS`.
